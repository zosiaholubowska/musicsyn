...
def create_df():
    ...
    at_boundary_grouped = at_boundary.groupby(['subject', 'condition', 'signal_theory']).size().unstack(fill_value=0)
    ...
    no_boundary_grouped = no_boundary.groupby(['subject', 'condition', 'signal_theory']).size().unstack(fill_value=0)
...
def plot_group(condition):
    ...
    fig.suptitle('Comparison between two conditions - group')
    ...
    plt.savefig(f'{path}/plots/{condition}_main_results.png', dpi=300)
    plt.show()
...
######## SINGLE PARTICIPANT
def plot_single(subject, condition):
    path = os.getcwd()
...
#     path = os.getcwd()
#     main = pandas.read_csv(path + f"/Results/results_postprocessed.csv")
#     main.loc[main['condition'] == 'training', 'condition'] = 'training'
#     vc_data = pandas.read_csv(path + f"/Results/results.csv")
#     vc_data_filtered = vc_data[vc_data['answer'] == 1]
